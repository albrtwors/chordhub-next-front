...
export default function UserDropdown() {
  ...
  const [pfp, setPfp]:any = useState(false)
  const [userName, setUserName] = useState(false)
  const [email, setEmail] = useState(false)


  const handleLogout = ()=>{
    axios.get(`${routeClient()}/api/profile/logout`).then(res=>{
      setTimeout(()=>{window.location.href=`${routeClient()}/signin`}, 1000)
    })
  }
  ...
  useEffect(()=>{
    try{
      ...
      axios.get(`${routeClient()}/api/profile/avatar`).then(res=>{
        const pfpData = res.data
        console.log(pfpData)
        setPfp(pfpData)
      })
  ...
  return (
    <div className="relative">
      ...
      >
          ...
          {/* <div className={`w-10 bg-cover   h-10 rounded-full bg-sky-400 me-3`}></div> */}
          <img className="w-10 h-10 rounded-full me-5" src={pfp ? pfp : 'none' }></img>
