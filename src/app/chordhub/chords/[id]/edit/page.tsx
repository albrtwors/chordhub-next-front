...
export default function Page(){
    ...
    const [versionName, setVersionName]: any = useState(null)
    const {id} = useParams()
    ...
    useEffect(()=>{
        axios.get(`${routeClient()}/api/auth/chords/${id}/edit`).then(res=>{
           setSongScript(songJsonToText(res.data.song.structure))
           setScriptJson(res.data.song.structure)
           setSongName(res.data.song.songs.name)
           setSongId(res.data.song.songs.id)
           setVersionName(res.data.song.name)
        })
    ...
    const handleSubmit = async (e:any)=>{
      ...
      objectForm = {...objectForm, structure:scriptJson, song_id:songId}

      Alert.LoadingAlert()
      
      const res:any = await axios.patch(`${routeClient()}/api/auth/chords/${id}/update`, objectForm, 
        {headers: {
